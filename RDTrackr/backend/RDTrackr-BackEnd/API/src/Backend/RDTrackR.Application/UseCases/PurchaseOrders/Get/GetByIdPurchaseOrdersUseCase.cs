...
{
    public class GetByIdPurchaseOrdersUseCase : IGetByIdPurchaseOrdersUseCase
    {
        private readonly IPurchaseOrderReadOnlyRepository _repository;
        private readonly ILoggedUser _loggedUser;
        private readonly IMapper _mapper;

        public GetByIdPurchaseOrdersUseCase(
        ...
        public async Task<List<ResponsePurchaseOrderJson>> Execute(long id)
        {
            var loggedUser = await _loggedUser.User();
            var orders = await _repository.GetByIdAsync(id, loggedUser);
